...
import { Calculator, Users, TrendingUp, Search, ExternalLink, Loader2, Mail, Phone } from "lucide-react";
...
export default function AdminSimuladorLeads() {
  ...
  const load = async () => {
    ...
    const { data } = await supabase
      .from("simulator_leads" as any)
      .select("*")
      .order("created_at", { ascending: false });
    setLeads((data as unknown as SimLead[]) || []);
    setLoading(false);
  };
  ...
  const updateStatus = async (id: string, status: string) => {
    await supabase.from("simulator_leads" as any).update({ status }).eq("id", id);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Simulador de Renda Fixa</h1>
          <p className="text-muted-foreground text-sm mt-0.5">Leads capturados pelo simulador público</p>
        </div>
        <a href="/ferramentas/simulador-de-renda-fixa" target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 text-xs font-medium text-novare-blue hover:underline">
          <ExternalLink className="h-3.5 w-3.5" /> Ver simulador público
        </a>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Novos",        value: stats.novos,      icon: Calculator, color: "text-amber-500" },
      ...
      {loading ? (
        ...
        <Card><CardContent className="p-10 text-center text-muted-foreground text-sm">
          Nenhum lead encontrado. Compartilhe o simulador público para captar e-mails!
        </CardContent></Card>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border/50">
                  ...
                  <th className="text-right font-semibold px-3 py-3">Simulação</th>
                  <th className="text-right font-semibold px-3 py-3">Líquido projetado</th>
                ...
                {filtered.map((l) => (
                  <tr key={l.id} className="border-b border-border/30 last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-3 py-3 text-right text-xs text-muted-foreground">
                      <p>{brl(l.valor_inicial)} + {brl(l.aporte_mensal)}/mês</p>
                      <p>{l.prazo_meses}m · {l.tipo_principal}</p>
                    </td>
                    <td className="px-3 py-3 text-right tabular-nums font-bold text-emerald-600">{brl(l.resultado_liquido)}</td>
