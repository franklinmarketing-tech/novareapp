...
import { PiggyBank, Users, TrendingUp, Search, ExternalLink, Loader2, Mail, Phone } from "lucide-react";
...
export default function AdminAposentadoriaLeads() {
  ...
  const updateStatus = async (id: string, status: string) => {
    await supabase.from("retirement_leads" as any).update({ status }).eq("id", id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Calculadora de Aposentadoria</h1>
          <p className="text-muted-foreground text-sm mt-0.5">Leads capturados pela calculadora pública</p>
        </div>
        <a href="/ferramentas/calculadora-de-aposentadoria" target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 text-xs font-medium text-novare-blue hover:underline">
          <ExternalLink className="h-3.5 w-3.5" /> Ver calculadora pública
        </a>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Novos",        value: stats.novos,      icon: PiggyBank,  color: "text-amber-500" },
      ...
      {loading ? (
        ...
        <Card><CardContent className="p-10 text-center text-muted-foreground text-sm">
          Nenhum lead ainda. Compartilhe a calculadora pública para captar e-mails!
        </CardContent></Card>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border/50">
                  ...
                  <th className="text-right font-semibold px-3 py-3">Perfil</th>
                  <th className="text-right font-semibold px-3 py-3">Meta (FIRE)</th>
                  <th className="text-right font-semibold px-3 py-3">Independência</th>
                ...
                {filtered.map((l) => (
                  <tr key={l.id} className="border-b border-border/30 last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-3 py-3 text-right text-xs text-muted-foreground">
                      <p>{l.idade_atual}→{l.idade_aposentadoria} anos</p>
                      <p>{brl(l.patrimonio_atual)} + {brl(l.aporte_mensal)}/mês</p>
                    </td>
                    <td className="px-3 py-3 text-right tabular-nums font-bold text-slate-700 dark:text-slate-300">{brl(l.fire_number)}</td>
                    <td className="px-3 py-3 text-right text-xs">
                      <span className="font-bold text-emerald-600">{l.idade_independencia ? `${l.idade_independencia} anos` : "—"}</span>
                      <p className="text-muted-foreground">renda {brl(l.renda_desejada)}/mês</p>
                    </td>
