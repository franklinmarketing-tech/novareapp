...
import { UserCog, Users, CheckCircle2, Search, ExternalLink, Loader2, Mail, Phone } from "lucide-react";
...
const PERFIL_COLORS: Record<string, string> = {
  Conservador: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-400",
  Moderado: "bg-blue-100 text-blue-700 dark:bg-blue-950/50 dark:text-blue-400",
  Arrojado: "bg-orange-100 text-orange-700 dark:bg-orange-950/50 dark:text-orange-400",
  Agressivo: "bg-rose-100 text-rose-700 dark:bg-rose-950/50 dark:text-rose-400",
};

const fmtDate = (d: string) => new Date(d).toLocaleDateString("pt-BR");

export default function AdminPerfilLeads() {
  ...
  const updateStatus = async (id: string, status: string) => {
    await supabase.from("profile_leads" as any).update({ status }).eq("id", id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Teste de Perfil de Investidor</h1>
          <p className="text-muted-foreground text-sm mt-0.5">Leads do teste de suitability</p>
        </div>
        <a href="/ferramentas/teste-de-perfil" target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 text-xs font-medium text-novare-blue hover:underline">
          <ExternalLink className="h-3.5 w-3.5" /> Ver teste público
        </a>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Novos",        value: stats.novos,      icon: UserCog,     color: "text-amber-500" },
      ...
      {loading ? (
        ...
        <Card><CardContent className="p-10 text-center text-muted-foreground text-sm">
          Nenhum lead ainda. Compartilhe o teste público para captar e-mails!
        </CardContent></Card>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border/50">
                  ...
                  <th className="text-center font-semibold px-3 py-3">Perfil</th>
                  <th className="text-center font-semibold px-3 py-3">Pontuação</th>
                ...
                {filtered.map((l) => (
                  <tr key={l.id} className="border-b border-border/30 last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-3 py-3 text-center">
                      {l.perfil && <span className={cn("text-xs font-bold px-2.5 py-1 rounded-full", PERFIL_COLORS[l.perfil] || "bg-muted")}>{l.perfil}</span>}
                    </td>
                    <td className="px-3 py-3 text-center tabular-nums text-muted-foreground">{l.score != null ? `${l.score}/24` : "—"}</td>
