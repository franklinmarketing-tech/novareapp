...
import { TrendingDown, Users, CheckCircle2, Search, ExternalLink, Loader2, Mail, Phone } from "lucide-react";
...
const mesesLabel = (m: number | null) => {
  if (m == null) return "—";
...
export default function AdminDividasLeads() {
  ...
  const updateStatus = async (id: string, status: string) => {
    await supabase.from("debt_leads" as any).update({ status }).eq("id", id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Saia do Vermelho</h1>
          <p className="text-muted-foreground text-sm mt-0.5">Leads da calculadora de quitação de dívidas</p>
        </div>
        <a href="/ferramentas/saia-do-vermelho" target="_blank" rel="noopener noreferrer"
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Novos",        value: stats.novos,      icon: TrendingDown, color: "text-amber-500" },
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border/50">
                  ...
                  <th className="text-right font-semibold px-3 py-3">Dívida total</th>
                  <th className="text-right font-semibold px-3 py-3">Extra/mês</th>
                  <th className="text-right font-semibold px-3 py-3">Quita em</th>
              ...
              <tbody>
                {filtered.map((l) => (
                  <tr key={l.id} className="border-b border-border/30 last:border-0 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-3 py-3 text-right tabular-nums font-bold text-rose-600">{brl(l.total_divida)}</td>
                    <td className="px-3 py-3 text-right tabular-nums text-muted-foreground">{brl(l.extra_mensal)}</td>
                    <td className="px-3 py-3 text-right text-xs">
                      <span className="font-bold text-emerald-600">{mesesLabel(l.meses_avalanche)}</span>
                      <p className="text-muted-foreground">{l.num_dividas} dívidas</p>
                    </td>
