...
export const StepSeguros = ({ data, onChange }: Props) => {
  ...
  const remove = (i: number) => {
    ...
    if (hasData && !window.confirm("Excluir este seguro?")) return;
          ...
          <div key={item.id ?? i} data-item-id={item.id} className="p-5 rounded-2xl border border-border/60 bg-card space-y-4 shadow-soft hover:shadow-elevated hover:border-border transition-all duration-200">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 truncate text-[0.9375rem] font-semibold text-foreground">{cleanValue(item.type) || "Novo seguro"}{cleanValue(item.provider) ? ` · ${cleanValue(item.provider)}` : ""}</p>
                <p className="text-xs font-medium text-muted-foreground">{formatMoney(item.monthly_premium)}/mês · Cobertura {formatMoney(item.coverage_amount)}</p>
              </div>
