...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: isSuper } = await callerClient.rpc("has_role", {
      _user_id: userId,
      _role: "super_admin",
    });
    if (!isAdmin && !isSuper) {
    ...
    try {
      ...
      if (!res.ok) {
        emailStatus = "failed";
        errorMessage = json?.message || "Falha no envio";
      }
    } catch (e) {
      emailStatus = "failed";
    ...
    // Registrar histórico
    const { error: replyInsErr } = await admin.from("newsletter_lead_replies").insert({
      lead_id: leadId,
      replied_by_user_id: userId,
      replied_by_email: callerEmail,
      subject,
      message,
    ...
    console.error("newsletter-reply error:", err);
